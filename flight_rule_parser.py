...
# text parser
def parse_text(text,source,patterns,rules,online_info):
    ...
    for p in patterns['default']:
        ...
        if len(p.output_keys_common) > 0:
            for i in range(len(p.output_keys_common)):
                ...
                pInstance.common_output_content[i] = eval(key_parser[key_name])(pInstance,key_index)
        ...
        if len(p.output_keys_eng) > 0:
            for i in range(len(p.output_keys_eng)):
                ...
                pInstance.eng_output_content[i] = eval(key_parser[key_name])(pInstance,key_index)
...
#-----------------------------------------------------ignore-----------------------#    
    ...
    for s in strs:
        ...
        # 一句话可能同时匹配多个pattern，所以需要将每个pattern都尝试匹配一次  patterns[option] 有28个选项。
        for p in patterns['option']:                
            finds = p.pattern.findall(s) 
            if len(finds) == 0:
                continue
            else:
                is_recoginized = True                   # 这里面的len(finds)==1
               ...
               # print "-------output_keys_common is over"
                ...
                # 暂时不做英文
               # print 'Output String: %s'%pInstance.output_common
